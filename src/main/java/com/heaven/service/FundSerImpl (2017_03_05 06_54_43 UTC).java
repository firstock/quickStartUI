...
import com.heaven.dao.FundDaoImpl;
import com.heaven.dto.FundBean;

@Service
public class FundSerImpl implements FundSer {
	@Autowired
	private FundDaoImpl fundDAO;

	@Override
	public ModelAndView getMyFund(HttpServletRequest request) {
		List<FundBean> fundlist;
		...
		int listcount=fundDAO.getFundCount(email);
		...
		fundlist=fundDAO.getMyFundList(params);				
		...
		mav.addObject("page", page);
	   	...
	   	mav.setViewName("member/MyDonation");
		return mav;
	}
//by J.K.
	public int addFund(String title, String email, int money) {
		System.out.println("service");
		FundBean bean=new FundBean();
		bean.setUSER_EMAIL(email);
		bean.setPROJECT_TITLE(title);
		bean.setFUND_DONATION(money);
		System.out.println(bean);
		int supporters = fundDAO.insertFund(bean);
		return supporters;
	}
